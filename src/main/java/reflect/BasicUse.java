 ...
 *  2. 解决什么问题
 *      运行时判断对象所属的类，构造任意类的对象，操作对象的属性和方法，获取类对应包/父类/实现的接口/方法注解权限返回值参数列表
 *  3. 相关API
 *      （1）{@link java.lang.Class} 该类对象代表运行中java中的类和接口，该对象随着类的加载由JVM自动创建，对应一个.class文件
 *      （2）{@link java.lang.reflect.Method} 类的方法
 *      （3）{@link java.lang.reflect.Field} 类的s属性
 *      （4）{@link java.lang.reflect.Constructor} 类的构造方法
 *  4. 优点和缺点
 *      （1）运行时确定类型，降低了代码与类型的耦合性，使得程序更加灵活，可扩展
 *      （2）运行时确定类型导致程序在编译时（JIT），编译器无法对其进行优化，导致性能较差
 *      （3）通过反射操作对象，需要进行额外的操作，例如调用方法需要检查方法可见性、校验参数、参数封装以及请求JVM到方法区查找对应方法
 * */
public class BasicUse {
    ...
    /**
     * 类加载机制（classloader）：将Class字节码文件加载到内存中，将静态数据转化为方法区的运行数据结构后，在堆生成一个对应java.lang.Class对象
    ...
    public void testClassLoader() throws ClassNotFoundException, IOException {
        ...
        //Application ClassLoader的扫名路径为classpath(未打包时的src(maven会变成resources文件夹为根目录),打包后的target/classes)
        BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(Objects.requireNonNull(this.getClass().getClassLoader().getResourceAsStream("BasicUse.java"))));
