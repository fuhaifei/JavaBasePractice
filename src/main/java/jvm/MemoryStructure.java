 ...
 *      * 虚拟机规范约定：虚拟机栈大小可以时固定大小（StackoverflowError）或者动态扩展（OutOfMemoryError）
 *      * 栈帧的内部结构：
 ...
 *              * java虚拟机执行引擎是基于栈的执行引擎，这里的栈就是操作数栈
 *          * 动态链接：一个指向运行时常量池中该栈帧所属方法的引用
 *              * 静态/动态链接：是否在编译期间将符号引用转化为直接引用，如果编译期间可确定则为静态链接（早期绑定），运行时确定动态链接（晚期绑定）
 *              * 早期/晚期绑定：编译时替换/运行时替换
 *              * 动态/静态语言：对类型的检查是在编译期还是运行期,动态->变量没有类型，变量值有类型
