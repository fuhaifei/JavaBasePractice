 ...
 *        * ZGC：解决CMS和G1垃圾回收停顿时间问题
 *              * 着色指针
 *                  * ZGC将java 64为地址指针划分为了（16位固定，1位未使用，4位元数据（着色指针），42位地址指针）
 *                  * 只有低42位代表逻辑地址，这也是ZGC说自己只支持4TB的原因
 *                  * 着色指针将逻辑空间划分了三种视图，三种视图映射到同一个物理空间
 *                      * Marked0、Marked1和Remapped
 *                      * 将第42、43、44位设置为1
 *                      * M0,M1用来区分是上次还是本次标记
 *              * 读屏障：在字节码中插入特定代码，在访问对象时执行特定的逻辑
 *                  * 判断当前对象视图是否=地址视图，相等则直接执行下一步
 *                  * 不相等判断是否正在转移，如果正在转移，这进行转移
 *                  * 否则判断是否为强引用，是进入标记阶段，否则进入重定位
 *              * ZGC执行流程（初始指针状态均为Remapped）
 *                  * 初始标记阶段：从 GC Roots 出发，找出 GC Roots 直接引用的对象，放入活跃对象集合，STW时间只与CGRoots数量相关
 *                  * 并发标记阶段：GC标记线程/用户线程（基于读屏障）访问到对象将对象 从Remapped->M0视图
 *                      * 真并发，和用户线程一起标记
 *                      * 标记结束后，Marked0 = 活跃的，Remapped = 不活跃
 *                  * 再标记：标记因为引用关系变化导致的漏标存活对象，STW时间较短
 *                  * 初始转移：将CGRoots直接引用的对象进行复制转移，STW时间只与CGRoots数量相关
 *                  * 并发转移：GC线程/用户线程（基于读屏障）访问若对象视图为M0,则复制转移，并修改视图为Remapped
 *                  * 重定位：将转移对象指针替换为新地址指针
 *              * 还是分region，但是没有分代的逻辑
 *
 *
